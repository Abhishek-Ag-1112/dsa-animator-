...
Return ONLY the raw JSON output matching the schema. Do not write any think blocks, <think> tags, explanation, reasoning, or markdown code blocks. Start your response with '{' and end with '}'.`;
        ...
        try {
          ...
          if (content) {
            try {
              ...
              // If we reached here, the JSON is completely valid!
              parsedData = parsed;
              modelSucceeded = true;
              break; 
            } catch (parseError: any) {
              lastErrorText = `JSON Parse Error: ${parseError.message || String(parseError)}`;
              lastStatus = 422;
              console.warn(`JSON parsing failed for model ${model} (json=${useJsonFormat}): ${lastErrorText}. Raw response: ${content}`);
            }
          } else {
    ...
    if (!parsedData) {
      return NextResponse.json({
        success: false,
        error: 'All candidate Groq models failed or returned invalid JSON',
